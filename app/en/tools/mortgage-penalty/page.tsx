...
export default function Page() {
  ...
  const [comparisonRatePct, setComparisonRatePct] = useState<number>(3.39); // lender's comparable posted/discounted rate for remaining term
  ...
  function exportCSV() {
    const rows: Array<Array<string | number>> = [
      ...
      ["Remaining Term (months)", remainingMonths],
      ...
      ["—", "—"],
      ["Notes", "Results are estimates; lender methods vary (posted rates, discount method, exact day-count)."],
    ];
  ...
  return (
    <ToolShell
      ...
      subtitle="Estimate early-payout penalties using 3-month interest and a simple IRD model. Export a branded PDF or CSV."
      lang="en"
    >
      {/* Toolbar */}
      ...
      <form className="grid xl:grid-cols-3 gap-6">
        ...
        <section className="rounded-2xl border border-brand-gold bg-white p-5 grid gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-brand-blue mb-1">Remaining Term (months)</label>
              <select
                className="w-full rounded-xl border border-brand-gold/60 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-brand-gold"
        ...
        <section className="rounded-2xl border border-brand-gold bg-white p-5">
          ...
          <p className="text-sm text-brand-blue/80 mb-3">
            IRD estimates can vary by lender. We use a simple approach:{" "}
            <b>Balance × (Contract Rate − Comparison Rate) × (Remaining Months ÷ 12)</b>.
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-brand-blue mb-1">Admin / Discharge Fee (optional, CAD)</label>
              <input
                type="number"
                min={0}
                inputMode="decimal"
              ...
              />
              <p className="text-xs text-brand-blue/70 mt-1">Some lenders add a fixed processing or discharge fee.</p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-brand-gold bg-white p-5 avoid-break">
          ...
          <details className="mt-4 rounded-xl border border-brand-gold/40 bg-brand-beige/40 p-4">
            ...
            <ul className="list-disc ml-5 mt-2 text-sm text-brand-blue/80 space-y-1">
              <li><b>3-Month Interest</b> = Balance × Contract Rate × (3/12).</li>
              <li><b>IRD (simple)</b> = Balance × (Contract − Comparison) × (Remaining Months ÷ 12).</li>
              <li>Lenders may use posted vs. discounted rate methods, exact day-counts, or other rules. Your agreement governs.</li>
              <li>Some products (open, blend-and-extend, portability) can change or reduce penalties.</li>
              <li>This tool provides an estimate only and is not financial advice.</li>
            </ul>
          </details>
        </section>
      </form>
      ...
      <div className="mt-8 grid lg:grid-cols-2 gap-6">
        <section className="rounded-2xl border border-brand-gold bg-white p-5 avoid-break">
          ...
          <div className="text-sm space-y-1">
            <div className="flex justify-between"><span>Outstanding Balance</span><span className="font-medium">{money(outstandingBalance)}</span></div>
            <div className="flex justify-between"><span>Contract Rate</span><span className="font-medium">{contractRatePct.toFixed(2)}%</span></div>
            <div className="flex justify-between"><span>Penalty (3 months)</span><span className="font-semibold">{money(penalty3mo)}</span></div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-brand-gold bg-white p-5 avoid-break">
          ...
          <div className="text-sm space-y-1">
            <div className="flex justify-between"><span>Outstanding Balance</span><span className="font-medium">{money(outstandingBalance)}</span></div>
            <div className="flex justify-between"><span>Contract Rate</span><span className="font-medium">{contractRatePct.toFixed(2)}%</span></div>
            <div className="flex justify-between"><span>Comparison Rate</span><span className="font-medium">{comparisonRatePct.toFixed(2)}%</span></div>
            <div className="flex justify-between"><span>Remaining Term</span><span className="font-medium">{remainingMonths} months</span></div>
            <div className="flex justify-between"><span>Penalty (IRD)</span><span className="font-semibold">{money(penaltyIRD)}</span></div>
          </div>
        </section>
      </div>
