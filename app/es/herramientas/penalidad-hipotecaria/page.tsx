...
export default function Page() {
  ...
  // Derivados
  const { penalty3mo, penaltyIRD, basePenalty, ruleText, totalWithFees } =
    useMemo(() => {
      ...
      <form className="grid xl:grid-cols-3 gap-6">
        ...
        <section className="rounded-2xl border border-brand-gold bg-white p-5">
          ...
          <p className="text-sm text-brand-blue/80 mb-3">
            ...
            Para un cálculo más cercano, ingresa la tasa comparable actual de tu prestamista para el
            plazo restante.
          </p>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div>
              ...
              <p className="text-xs text-brand-blue/70 mt-1">
                Usualmente la tasa publicada/con descuento del prestamista para un plazo que coincide
                con tus meses restantes.
              </p>
            </div>

            <div>
              <label className="block text-sm font-medium text-brand-blue mb-1">
                Tarifa administrativa / liberación (opcional, CAD)
              </label>
              <input
                type="number"
                min={0}
              ...
              />
              <p className="text-xs text-brand-blue/70 mt-1">
                Algunos prestamistas agregan un cargo fijo de tramitación o liberación.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-brand-gold bg-white p-5 avoid-break">
          ...
          <details className="mt-4 rounded-xl border border-brand-gold/40 bg-brand-beige/40 p-4">
            ...
            <ul className="list-disc ml-5 mt-2 text-sm text-brand-blue/80 space-y-1">
              <li>
                <b>Interés 3 meses</b> = Saldo × Tasa contractual × (3/12).
              </li>
              <li>
                <b>IRD (simple)</b> = Saldo × (Contrato − Comparativa) × (Meses restantes ÷ 12).
              </li>
              <li>
                El método exacto puede usar tasas publicadas vs. con descuento, cómputos de días u
                otras reglas. Tu contrato manda.
              </li>
              <li>
                Algunos productos (open, blend-and-extend, portabilidad) pueden cambiar o reducir
                penalidades.
              </li>
      ...
      <div className="mt-8 grid lg:grid-cols-2 gap-6">
        <section className="rounded-2xl border border-brand-gold bg-white p-5 avoid-break">
          ...
          <p className="text-sm text-brand-blue/80 mb-2">
            Normalmente aplicado a hipotecas variables o cuando resulta mayor para penalidades de tasa
            fija.
          </p>
          <div className="text-sm space-y-1">
            <div className="flex justify-between">
              <span>Saldo pendiente</span>
              <span className="font-medium">{money(outstandingBalance)}</span>
            </div>
            <div className="flex justify-between">
              <span>Tasa contractual</span>
              <span className="font-medium">{contractRatePct.toFixed(2)}%</span>
            </div>
            <div className="flex justify-between">
              <span>Penalidad (3 meses)</span>
        ...
        <section className="rounded-2xl border border-brand-gold bg-white p-5 avoid-break">
          ...
          <div className="text-sm space-y-1">
            ...
            <div className="flex justify-between">
              <span>Tasa de comparación</span>
              <span className="font-medium">{comparisonRatePct.toFixed(2)}%</span>
            </div>
            <div className="flex justify-between">
              <span>Plazo restante</span>
              <span className="font-medium">{remainingMonths} meses</span>
            </div>
            <div className="flex justify-between">
              <span>Penalidad (IRD)</span>
