...
import { getAllArticles, type ArticleMeta, type Lang } from "@/lib/getArticles";
...
export default async function RelatedArticles({
  ...
  if (category) {
    related = pool.filter((a) => a.category && a.category === category);
  }
  if (related.length < max && tags?.length) {
    const need = max - related.length;
    const tagMatches = pool.filter(
      (a) => a.tags && a.tags.some((t) => tags.includes(t))
    );
  ...
  return (
    <section aria-labelledby="related-heading" className="mt-12">
      <h2 id="related-heading" className="text-xl font-semibold mb-4">{heading}</h2>
      <ul className="grid gap-4 sm:grid-cols-2">
        {related.map((a) => (
          <li key={a.slug} className="rounded-2xl border border-brand-gold/30 bg-white p-4 shadow-sm hover:bg-neutral-50 dark:hover:bg-neutral-900">
